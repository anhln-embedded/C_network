...
struct block_config_s
{
     ...
     char mac[18];
     int time_state;
     list_head_t block_time;
};

list_head_t l_block_config = {&(l_block_config), &(l_block_config)};
...
void add_block_config(const char *url, const char *mac)
{
     ...
     if (cfg)
     {
          do
          {
               (&cfg->block_time)->next = (&cfg->block_time);
               (&cfg->block_time)->prev = (&cfg->block_time);
          } while (0);
          strncpy(cfg->url, url, 128);
          strncpy(cfg->mac, mac, 18);
          cfg->time_state = 1;
...
void print_all_configs()
{
     ...
     for (cfg = ((typeof(*cfg) *)((char *)((&l_block_config)->next) - (unsigned long)(&((typeof(*cfg) *)0)->list))); &cfg->list != (&l_block_config); cfg = ((typeof(*cfg) *)((char *)(cfg->list.next) - (unsigned long)(&((typeof(*cfg) *)0)->list))))
     {
          printf("Config: URL=%s, MAC=%s, TimeState=%d\n", cfg->url, cfg->mac, cfg->time_state);

          for (bt = ((typeof(*bt) *)((char *)((&cfg->block_time)->next) - (unsigned long)(&((typeof(*bt) *)0)->list))); &bt->list != (&cfg->block_time); bt = ((typeof(*bt) *)((char *)(bt->list.next) - (unsigned long)(&((typeof(*bt) *)0)->list))))
          {
...
void free_all_configs()
{
     ...
     for (p = (&l_block_config)->next, n = p->next; p != (&l_block_config); p = n, n = p->next)
     {
          cfg = ((block_config_t *)((char *)(p) - (unsigned long)(&((block_config_t *)0)->list)));

          list_head_t *tp, *tn;
          for (tp = (&cfg->block_time)->next, tn = tp->next; tp != (&cfg->block_time); tp = tn, tn = tp->next)
          {
               block_time_t *bt = ((block_time_t *)((char *)(tp) - (unsigned long)(&((block_time_t *)0)->list)));
               list_del(tp);
               free(bt);
          }
